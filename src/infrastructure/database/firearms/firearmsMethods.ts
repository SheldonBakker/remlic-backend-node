...
const CONTEXT = 'FIREARMS_SERVICE';

export async function getFirearmsByUserId(
  userId: string,
  ...
  const items = data.map((row) => mapToFirearm(row));
  const pagination = PaginationUtil.buildPagination(items, params.limit);

  return { items, pagination };
}

export async function getFirearmById(firearmId: string, userId: string): Promise<IFirearm> {
  const data = await db
    .select()
    .from(firearms)
  ...
  if (!data) {
    Logger.warn(CONTEXT, `Firearm not found (firearmId: ${firearmId})`);
    throw new HttpError(HTTP_STATUS.NOT_FOUND, 'Firearm not found');
  }

  return mapToFirearm(data);
}

export async function createFirearm(data: ICreateFirearmData): Promise<IFirearm> {
  try {
...
export async function updateFirearm(data: IUpdateFirearmData): Promise<IFirearm> {
  ...
  if (!existing) {
    Logger.warn(CONTEXT, `Firearm not found for update (firearmId: ${data.id})`);
    throw new HttpError(HTTP_STATUS.NOT_FOUND, 'Firearm not found');
  }
...
export async function deleteFirearm(firearmId: string, userId: string): Promise<void> {
  const existing = await db
    .select({ id: firearms.id })
    .from(firearms)
    .where(and(eq(firearms.id, firearmId), eq(firearms.profile_id, userId)))
  ...
  if (!existing) {
    Logger.warn(CONTEXT, `Firearm not found for deletion (firearmId: ${firearmId})`);
    throw new HttpError(HTTP_STATUS.NOT_FOUND, 'Firearm not found');
  }
...
function mapToFirearm(row: typeof firearms.$inferSelect): IFirearm {
  return {
    id: row.id,
