...
const CONTEXT = 'CERTIFICATES_SERVICE';
...
export async function getCertificatesByUserId(
  ...
  const items = data.map((row) => mapToCertificate(row));
  const pagination = PaginationUtil.buildPagination(items, params.limit);

  return { items, pagination };
}

export async function getCertificateById(certificateId: string, userId: string): Promise<ICertificate> {
  const data = await db
    .select()
    .from(certificates)
  ...
  if (!data) {
    Logger.warn(CONTEXT, `Certificate not found (certificateId: ${certificateId}, userId: ${userId})`);
...
export async function createCertificate(data: ICreateCertificateData): Promise<ICertificate> {
  ...
  return mapToCertificate(certificate);
}

export async function updateCertificate(data: IUpdateCertificateData): Promise<ICertificate> {
  ...
  if (!existing) {
    Logger.warn(CONTEXT, `Certificate not found for update (certificateId: ${data.id}, userId: ${data.profile_id})`);
    throw new HttpError(HTTP_STATUS.NOT_FOUND, 'Certificate not found');
  }
...
export async function deleteCertificate(certificateId: string, userId: string): Promise<void> {
  ...
  if (!existing) {
    Logger.warn(CONTEXT, `Certificate not found for deletion (certificateId: ${certificateId}, userId: ${userId})`);
    throw new HttpError(HTTP_STATUS.NOT_FOUND, 'Certificate not found');
  }
...
function mapToCertificate(row: typeof certificates.$inferSelect): ICertificate {
  return {
    id: row.id,
