...
import {
  ...
  getSubscriptionById,
  markRefunded,
} from '../infrastructure/database/subscriptions/subscriptionsMethods.js';
import { getPackageById } from '../infrastructure/database/packages/packagesMethods.js';
import type {
...
import db from '../infrastructure/database/databaseClient.js';
...
export async function handleWebhookEvent(payload: IPaystackWebhookPayload): Promise<void> {
  try {
    ...
    switch (event) {
      case 'charge.success': {
        ...
        if (metadata?.user_id && metadata.package_id) {
          const pkg = await getPackageById(metadata.package_id);
          ...
          const subscription = await getByPaystackCode(data.subscription_code);
          if (!subscription) {
            Logger.warn('SUBSCRIPTION_USE_CASES', `Subscription not found: ${data.subscription_code}`);
            break;
          }
          ...
          await renewSubscription(data.subscription_code, newEnd.toISOString());
        }
        break;
      }
    ...
    Logger.error('SUBSCRIPTION_USE_CASES', `Error processing webhook event: ${payload.event}`, error);
    throw error;
  }
}

export async function cancelUserSubscription(userId: string, subscriptionId: string): Promise<void> {
...
export async function refundSubscription(userId: string, subscriptionId: string): Promise<void> {
  const subscription = await getSubscriptionById(subscriptionId);
  ...
  if (daysSinceCreation > REFUND_WINDOW_DAYS) {
    throw new HttpError(
      ...
      `Refund period exceeded. Refunds are only available within ${REFUND_WINDOW_DAYS} days of purchase.`,
    );
  }
  ...
  const refundResult = await PaystackService.createRefund({
    ...
    merchant_note: `Refund requested by user within ${REFUND_WINDOW_DAYS}-day window`,
  });
...
export async function changePlan(
  ...
  const initResult = await initializeSubscription(userId, {
