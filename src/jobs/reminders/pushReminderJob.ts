...
import type { IBatchReminderItem, IBatchReminderResult } from '../../infrastructure/database/reminders/types';
...
export async function run(
  ...
  try {
    ...
    while (hasMore) {
      ...
      const profileIds = [...new Set(items.map((item: IBatchReminderItem) => item.profileId))];
      ...
      for (const item of items) {
        ...
        if (result.success) {
          ...
          const error: IJobError = { recordId: item.entityId, message: result.error ?? 'Push send failed' };
          errors.push(error);
      ...
      Logger.info(JOB_NAME, `Batch result: ${batchPushSent} sent, ${batchSkipped} skipped, ${errors.length} errors`);
